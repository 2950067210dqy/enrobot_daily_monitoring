...
飞马2的`/home/enrobot`来自共享NFS，应用日志目录中会同时出现其他服务器写入的文件。PDF解析层不提取飞马2的应用日志正文，也不展示其“当天WARN/ERROR日志”汇总项；这些内容不会进入日志去重、DeepSeek分析、PDF日志明细或知识图谱。飞马2自身的Docker日志和系统/进程日志仍正常解析。
...
3. 首页只展示磁盘使用率图表，横坐标为巡检时间，飞马1、飞马2和Client作为图例系列；一小时一次的CPU、内存和IO瞬时值不在PDF中展示或参与报告状态判断。磁盘会从df明细中提取全部真实业务磁盘及NFS挂载点，排除`/boot/efi`、tmpfs和Docker overlay等非业务或临时挂载，概要卡为每个挂载点分别显示“挂载点：使用率”和独立进度条，趋势图也按挂载点分别绘制；各服务器页面只展示该服务器的进程线程数和各日志目录大小变化图，并位于文字概要表之前；
4. AI/规则异常数量汇总；
...
新版 `server_health_client.sh` 是低权限轻量巡检，仍采集 CPU、内存/Swap、磁盘/inode、磁盘 I/O、指定进程和 Docker 可见性；PDF只展示磁盘/inode、进程、目录和业务检查结果，不展示CPU、内存或IO瞬时值，也不把这些瞬时指标的采集限制计入PDF覆盖状态。
...
每台服务器只将磁盘最高使用率及各挂载点使用率时间序列交给 DeepSeek，PDF展示磁盘指标的重要程度、判断依据、建议与操作分析。CPU、内存和IO瞬时值不发送给DeepSeek。磁盘指标并入该服务器第一批日志请求，不额外增加已有日志服务器的请求次数；服务器完全没有异常日志时才单独发送一次指标请求。
...
- `--ai required`：AI缺少Key、非鉴权类请求失败或响应异常时终止；Key无效或无权限时按下述熔断规则继续生成报告；
...
- 监控TXT中的异常日志范围是当天00:00至该次脚本执行时间，因此每台服务器只从巡检时间最晚的一份TXT提取日志；更早TXT用于磁盘、进程线程和目录大小趋势，CPU、内存和IO虽继续解析但不进入PDF或DeepSeek。日志去重及DeepSeek分析只基于最新TXT中的累计日志，不再叠加所有小时TXT；
...
- DeepSeek返回HTTP 401、403，或明确的无效Key鉴权错误时，本次报告立即熔断后续API请求；后续批次仍优先读取有效缓存，未命中缓存项直接使用规则预分类并继续生成PDF。无效Key熔断优先于`--ai required`，不会因为鉴权失败中断报告；鉴权错误原因只写执行日志，PDF不显示Key无效或鉴权失败信息；
