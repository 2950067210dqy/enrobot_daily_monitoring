#!/usr/bin/env bash
# 飞马2 Docker 容器监测：单据 javaapi* 期望 3 个，邮件 pyservice* 期望 3 个。

set -uo pipefail

HOST_LABEL="飞马2-101.132.174.165"
EXPECTED_JAVAAPI=3
EXPECTED_PYSERVICE=3
STATE_FILE="/var/tmp/feima2_docker_monitor.state"
LOCK_DIR="/var/tmp/feima2_docker_monitor.lock"
...
if (( ${#issues[@]} == 0 )); then
    if [[ "$last_state" == "ALERT" ]]; then
        ...
        message+=$'\n'
        message+="状态：单据 javaapi* ${EXPECTED_JAVAAPI} 个、邮件 pyservice* ${EXPECTED_PYSERVICE} 个，均存在且运行中。"
